...
pymain_run_python(int *exitcode)
{
    ...
    else if (main_importer_path != NULL) {  // if __name__ == '__main__': ?
        *exitcode = pymain_run_module(L"__main__", 0);
    }
    else if (config->run_filename != NULL) {    // python x.py
        *exitcode = pymain_run_file(config);
    }
    else {
        *exitcode = pymain_run_stdin(config);   // python < x.py
    }
...
pymain_main(_PyArgv *args)
{
    ...
    if (_PyStatus_IS_EXIT(status)) {    // 遇到异常退出 exit
        pymain_free();      // 释放内存
        return status.exitcode;
    }
    if (_PyStatus_EXCEPTION(status)) { // exception， error
        pymain_exit_error(status); // exist or abort
    }
    // 执行接下来的操作
    return Py_RunMain();
}
